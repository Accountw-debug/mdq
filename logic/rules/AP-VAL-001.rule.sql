...
plain_logic: >
  Finding, wenn ein Kreditor (kein CpD) eine USt-ID (STCEG) hat, deren erste zwei Zeichen
  ...
  deshalb kein Präfix-, sondern ein Formatfehler – sie gehören zu AP-VAL-002. Vorschlag:
...
why: >
  ...
  gefährdet den Vorsteuerabzug bzw. die Reverse-Charge-Behandlung der Eingangsrechnung.
  Auf Rechnungen ist sie Pflichtangabe nach § 14 UStG.
if_wrong: >
...
tests:
  ...
  hits:
    - "V:0000200025"   # DEF-0038: Sitzland DE, Praefix AT
    - "V:0000200033"   # DEF-0038: Sitzland DE, Praefix FR
    - "V:0000200060"   # DEF-0038: Sitzland DE, Praefix PL
  no_hits:
    - "V:0000200845"   # DEF-0004, Anker F-004: "DE811000004" passt zum Sitzland
    - "V:0000201330"   # DEF-0006, Anker F-006: "DE831504744" passt zum Sitzland
    - "V:0000200061"   # DEF-0042: Steuernummer im STCEG - Format-, kein Praefixfehler
  edge:
    # Dieselbe Datenlage auf der AR-Seite: diese Regel darf sie nicht liefern, das ist
    # AR-VAL-001. Ein fehlender Rollenfilter faellt hier auf.
    - "C:0000100040"   # DEF-0037
    # Die AR-Schwester kennt zwei Faelle: mit zweiter Quelle fuer das Soll (DEF-0036, die
    # Dublette liefert es) und ohne (DEF-0037). Auf der AP-Seite nennt DEF-0038 keine
    # Ueberschneidung - alle zehn Kreditoren sind der Fall "ohne zweite Quelle". Das Soll
    # entsteht deshalb hier wie dort deterministisch aus dem Sitzland, und die Quellenlage
    # sagt genau das. Der Fall "die Dublette liefert das Soll" kommt auf der AP-Seite mit
    # AP-DUP-001 (Sprint 4).
--- */
WITH expected AS (
    SELECT
    ...
    WHERE bp.role = 'VENDOR'
      ...
      -- (AP-VAL-002), kein Praefixfehler (D-058)
